...
start: function(self){
    ...
    var allCards = self.project.cards;
    console.log(allCards);
    var maxCards = Math.min(allCards.length, chevre.options.maxCardsPerSession);

    switch(self.options.mode){
    case StudyMode.NORMAL:
        //grab cards from the project that are eligible to be studied
        cardsToStudy = self.chooseCardsToStudy(allCards, maxCards);
        break;
    case StudyMode.CRAM:
    ...
    case StudyMode.PERFECTION:
        ...
        break;
    case StudyMode.CUSTOMIZE:
   		//they chose which ranks to study
   		var rawRankArray = $('#study-input-customize-grades').val(); //array of strings like "A", "B", etc.
   		//choose ALL cards that have one of these ranks
   		var cardList = rawRankArray.map(function(rankLetter){
   			//rankLetter is like "B", we need to get Rank["B"]
   			return allCards.filter(function(card){
   				return card.rank == Rank[rankLetter];
   			});
   		}).flatten();
   		if(cardList.length == 0){
   			//not enough cards, meaning they chose ranks with NO cards associated. Use all cards then.
   			cardList = allCards;
   		}
   		//this is the sampling frame... choose cards now
   		cardsToStudy = self.chooseCardsToStudy(cardList, maxCards);
   		//TODO FINISH
   		/*
   		 * Add fn that, given a list of cards and # to get, will choose that many based on which have the fewest reps left
   		 * Figure out how many cards will be in session [min(numCardsInProject, maxCardsFromSettings)] and allot flashcards per grade proportionally (eg if 50 A's and 30 B's and you go to study 8, you'll get 5 A's and 3 B's)
   		 * Reduce everyone's reps OUTSIDE of this switchcase
   		 */
    }
...
/**
 * Chooses an array of cards for you to study based on which have the fewest reps left. Decrements the repsLeft of all non-chosen cards.
 * @param {Card[]} cardList	The sampling frame of cards to pick from. length must be > 0.
 * @param {int} maxCards		How many cards to pick from that list. TODO add a minCards.
 * @return {Card[]}	an array of cards to study, length <= maxCards.
 */
chooseCardsToStudy: function(self, cardList, maxCards){
	var cardsWithMin = cardList.min('repsLeft', true); //list of cards that have the FEWEST repetitions left.
	var minRepsLeft = cardsWithMin[0].repsLeft; //the fewest reps left of any card in the deck (int).
	//Bring the # reps down by this number so that [2,2,5] => [0,0,3] and we can pull out those with 0 reps left.
	cardList.forEach(function(card){
		card.repsLeft = Math.max(0, card.repsLeft - minRepsLeft);
	});
	//Choose the cards to study
	var toStudy = cardsWithMin.sample(maxCards);
	return toStudy;
},
